...
//                System.out.println(desAttaque);
                ...
                // Tester afficher resultatsAttaque
                ...
                // Entrain de fix !!!
                resolveAttack(resultatsAttaque, tC, tS);

                // if tC regiment == 0, get it!
                if (tC.getNombreUnites() == 0) {
                    // L'attaquant a réussi l'attaque
                    System.out.println("Vous avez conquis le territoire.");
                    ...
                    attaquant.deplacerRegiment(tS, tC, nbRegiment);
                }
            } else if (resJ == 2) {
     ...
     * @param tC       Territoire Cible pour attaque
     * @return Le nombre de troupes restantes après l'attaque.
     */
    private void resolveAttack(List<De> resultatsAttaque, Territoire tC, Territoire tS) {
        // Lancez les dés de défense pour obtenir les résultats du défenseur.
        // 1 - 2 régiment : 1 desDefense
        // 3+ régiment : 2 desDefense

        int desDefense = demanderDesDefense(tC);
        List<De> resultatsDefense = getActualJoueur().lancerDes(desDefense);
        ...
        List<De> sortedAttackResults = resultatsAttaque.parallelStream()
                .sorted(Comparator.comparingInt(De::recupererValeur).reversed())
                .collect(Collectors.toList());

        // tester afficher sortedlist sortedAttackResults
        List<Integer> l1 = new ArrayList<Integer>();
        for (De d : sortedAttackResults) {
            l1.add(d.recupererValeur());
        }
        System.out.println("sortedAttackResults" + Arrays.toString(l1.toArray()));
        // fin test
        ...
        // tester afficher sortedlist sortedDefenseResults
        List<Integer> l2 = new ArrayList<Integer>();
        for (De d : sortedDefenseResults) {
            l2.add(d.recupererValeur());
        }
        System.out.println("sortedDefenseResults" + Arrays.toString(l2.toArray()));
        // fin test

        // comparer 2 valeur max de des
        if (sortedAttackResults.get(0).recupererValeur() > sortedDefenseResults.get(0).recupererValeur()){
            System.out.println("L'attaque réussit ! Le défenseur est réduit de 1 régiment");
            tC.retirerRegiment(1);
        } else {
            System.out.println("L'attaque a échoué ! Vous perdez 1 régiment.");
            tS.retirerRegiment(1);
        }

        if (sortedDefenseResults.size() > 1){
            if (sortedAttackResults.get(1).recupererValeur() > sortedDefenseResults.get(1).recupererValeur()){
                System.out.println("L'attaque réussit ! Le défenseur est réduit de 1 régiment");
                tC.retirerRegiment(1);
            } else {
                System.out.println("L'attaque a échoué ! Vous perdez 1 régiment.");
    ...
    public int demanderDesDefense(Territoire tC){
        if ((tC.getNombreUnites() >= 1) && (tC.getNombreUnites() < 3)){
            return 1;
        } else if ((tC.getNombreUnites() >= 3)) {
            return 2;
        }
        return 0;
    }
